...
export default function PictionaryConfigScreen({ navigation, route }) {
  ...
  const addTeam = () => {
    if (teams.length < 8) {
      ...
      Alert.alert('Límite alcanzado', 'Máximo 8 equipos');
    }
  };
  ...
  const toggleCategory = (categoryId) => {
    if (selectedCategories.includes(categoryId)) {
      if (selectedCategories.length > 1) {
        ...
        Alert.alert('Error', 'Debe haber al menos una opción de categoría seleccionada');
      }
    } else {
